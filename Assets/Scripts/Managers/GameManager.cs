...
public class GameManager : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			int cameraType = cameraIndex % (m_Tanks.Length + 1);
			if (cameraType == m_Tanks.Length) {
				m_FollowCameraControl.Disable ();
				m_CameraControl.Active ();
			} else {
				m_CameraControl.Disable ();
				m_FollowCameraControl.tankManager = m_Tanks [cameraType];
				m_FollowCameraControl.Active();
    ...
    private IEnumerator RoundPlaying ()
    {
		...
		// While there is not one tank left... OneTankLeft
		while (!IsTimeUp())
        {
			int remaining = Mathf.CeilToInt(m_totalTime - Time.time + startTime);
			string time = "";
	...
	{
		if(playerNum>m_Tanks.Length||playerNum<=0)
		{
			return;
		}
		scores [playerNum]++;
